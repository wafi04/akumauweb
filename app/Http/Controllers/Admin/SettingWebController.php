...
class SettingWebController extends Controller
{
    ...
    public function saveSettingWeb(Request $request)
    {
        $request->validate([
            ...
            'logo_banner' => 'image:mimes:jpg,jpeg,png,webp',
            'logo_cs' => 'image:mimes:jpg,jpeg,png,webp',
            'url_wa' => 'required',
            'url_ig' => 'required',
            'url_tiktok' => 'required',
        ...
        if($request->file('logo_banner')){
            $file4 = $request->file('logo_banner');
            $folder4 = 'assets/logo';
            $file4->move($folder4, $file4->getClientOriginalName());      
            DB::table('setting_webs')->where('id', 1)->update([
                'logo_banner' => "/".$folder4."/".$file4->getClientOriginalName()
            ]);
        }
        
        if($request->file('logo_cs')){
            $file5 = $request->file('logo_cs');
            $folder5 = 'assets/logo';
            $file5->move($folder5, $file5->getClientOriginalName());      
            DB::table('setting_webs')->where('id', 1)->update([
                'logo_cs' => "/".$folder5."/".$file5->getClientOriginalName()
            ]);
        }
